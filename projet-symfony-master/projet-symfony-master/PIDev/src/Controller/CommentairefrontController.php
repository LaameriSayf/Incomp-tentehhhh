...
class CommentairefrontController extends AbstractController
{
    ...
    {        
        ...
        $GlobalUser = $entityManager->getRepository(GlobalUser::class)->find(2); 

        if (!$commentaire) {
            throw $this->createNotFoundException('Commentaire non trouvé');
        }

        if ($commentaire->getIdGlobalUser() === $GlobalUser && $commentaire->isJaime() === true) {
           
        }

        $commentaire->setJaime(true);
        $commentaire->setNejaimepas(false);
        $commentaire->setIdGlobalUser($GlobalUser);
        $entityManager->flush();
    ...
    {        
        ...
        if ($commentaire->getIdGlobalUser() === $GlobalUser && $commentaire->isNejaimepas() === true) {
    ...
    public function deleteCommentairefront(int $id, Request $request,Security $security): Response
    {
        ...
        $GlobalUserId = 2; 
        if ($user && $user->getUserIdentifier() !== $GlobalUserId) {
...
public function addCommentairefront($id, Request $request, Security $security): Response
{
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        $GlobalUserId = 2; // ID de l'GlobalUser statiquement défini à 3
        $user = $this->getDoctrine()->getRepository(GlobalUser::class)->find($GlobalUserId);
        if (!$user) {
            throw $this->createNotFoundException('GlobalUser non trouvé avec l\'identifiant '.$GlobalUserId);
        }
        ...
        $commentaire->setIdGlobalUser($user);
...
public function like($id, EntityManagerInterface $entityManager): Response
{
    ...
    $likeee = $entityManager->getRepository(Like::class)->findOneBy(['commentaire' => $commentaire->getId(), 'userr' => 2]);
    $dislikeee = $entityManager->getRepository(Dislike::class)->findOneBy(['commentaire' => $commentaire->getId(), 'userr' => 2]);

    if ($likeee == null) {
        $commentaire->setNblike($commentaire->getNblike() + 1);
        $like = new Like();
        $like->setCommentaire($commentaire);
        $user = $entityManager->getRepository(GlobalUser::class)->find(2);
        $like->setUserr($user);
...
public function dislike($id, EntityManagerInterface $entityManager): Response
{
    ...
    if ($dislikeee == null) {
        ...
        $user = $entityManager->getRepository(GlobalUser::class)->find(1);
